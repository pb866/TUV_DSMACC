...
*=============================================================================*
      ...
      SUBROUTINE interpol(x,y,kdata,n,nw,wl,jlab,lowbound,upbound,yg)

*-----------------------------------------------------------------------------*
*=  PURPOSE:                                                                 =*
*=  Combine interpolation routines and routines to add data points at edges  =*
*=  frequently used to map cross section and quantum yield data on           =*
*=  wavelength grid.                                                         =*
*-----------------------------------------------------------------------------*
*=  PARAMETERS:                                                              =*
*=  - x,y:      Real arrays of x and y data to be mapped onto grid      (IO) =*
*=  - kdata:    Arrays size of x and y (maximum number of data points)   (I) =*
*=  - n:        Actual number of data points                            (IO) =*
*=  - nw:       Number of intervals + 1 in working wavelength grid       (I) =*
*=  - wl:       Vector of lower limits of wavelength intervals in            =*
*=              working wavelength grid                                  (I) =*
*=  - jlab:     String with current photoreaction for error handling     (I) =*
*=  - lowbound/ REAL, values at the boundaries that are extented to          =*
*=    upbound:  the beginning/end of the TUV wavelength grid             (I) =*
*-----------------------------------------------------------------------------*

      IMPLICIT NONE
      INCLUDE '../params'

!     Input (INOUT is returned modifed):
      INTEGER, INTENT(IN) ::        kdata
      INTEGER, INTENT(INOUT) ::     n
      REAL(4), INTENT(INOUT) ::     x(kdata), y(kdata)
      INTEGER, INTENT(IN) ::        nw
      REAL(4), INTENT(IN) ::        wl(kw)
      CHARACTER(lcl), INTENT(IN) :: jlab
      REAL(4), INTENT(IN) ::        lowbound, upbound
!     Output:
      REAL(4), INTENT(OUT) ::       yg(kw)
!     Internal:
      INTEGER ::                    ierr

!     Add data points at edges, define boundary values
      CALL addpnt(x,y,kdata,n,x(1)*(1.-deltax),lowbound)
      CALL addpnt(x,y,kdata,n,              0.,lowbound)
      CALL addpnt(x,y,kdata,n,x(n)*(1.+deltax),upbound)
      CALL addpnt(x,y,kdata,n,          1.e+38,upbound)
!     Map data on wavelength grid
